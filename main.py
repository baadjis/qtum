# The tools we will need
from qiskit import QuantumRegister, ClassicalRegister, QuantumCircuit, Aer, execute  
 # Qiskit allows us to make beautiful plots 
from qiskit.visualization import plot_histogram                                     

# allows us to do quantum simulation of measurements 
M_simulator=Aer.backends(name='qasm_simulator')[0] 

def create_circuit():
    #create a quantum register
    qreg=QuantumRegister(2) # qreg is filled with two qubits 

    #create a classical register
    creg=ClassicalRegister(2) # creg is filled with two classical bits

    #finaly create our quantum circuit
    entangler=QuantumCircuit(qreg,creg) # we put our qreg and creg together to make our Quantum Circuit, called entangler here.
   
    return entangler

def apply_gates(circuit:QuantumCircuit):
    circuit.h(0) # Apply the Hadamard gate to the first qubit 
    circuit.cx(0,1) # Apply the CNOT gate with the first qubit as the control and second qubit as the target

def apply_mesures(circuit:QuantumCircuit):
    circuit.measure(0,0) # measure the first qubit and record it in the first classical bit
    circuit.measure(1,1) # measure the second qubit and record it in the second classical bit 

def draw_circuit(circuit:QuantumCircuit):
    #drawing our circuit
    circuit.draw(output='mpl',filename="circuit") 

def execute_measures(circuit:QuantumCircuit):

    #executes numerous measurements
    job=execute(circuit, M_simulator) 
    #gives us histogram results 
    hist=job.result().get_counts() 
    return hist

def main():
    etangler= create_circuit()
    apply_gates(etangler)
    apply_mesures(etangler)
    draw_circuit(etangler)
    hist=execute_measures(etangler)

    #plot a stunning visualization
    plot_histogram(data=hist, title="Maximally Entangled State",filename="hist")


if __name__=='__main__':
    main()
